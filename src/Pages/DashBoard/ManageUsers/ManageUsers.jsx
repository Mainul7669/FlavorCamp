...
const ManageUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery(['users'], async () => {
        const res = await fetch('https://assignment-12-server-one-theta.vercel.app/users')
        return res.json()
    })
    ...
    const handleMakeAdmin = user => {
        fetch(`https://assignment-12-server-one-theta.vercel.app/users/admin/${user._id}`, {
            method: 'PATCH'
    ...
    const handleMakeInstuctor = user => {
        fetch(`https://assignment-12-server-one-theta.vercel.app/users/instuctor/${user._id}`, {
            method: 'PATCH'
